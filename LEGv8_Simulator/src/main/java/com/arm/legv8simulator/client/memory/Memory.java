...
public class Memory {
     ...
     */
    public Memory(final int numInstructions) {
        staticDataSegmentOffset = TEXT_SEGMENT_OFFSET + (long) numInstructions * WORD_SIZE;
    ...
    public void storeDoubleWord(final long address, final long value) throws SegmentFaultException {
        ...
        for (int i = 0; i < DOUBLEWORD_SIZE; i++) {
            storeByte(address + i, buffer.getByte(i));
        }
    }
     ...
     * @throws SegmentFaultException
     */
    public long loadSignedWord(final long address) throws SegmentFaultException {
        boundsCheck(address, WORD_SIZE);
        for (int i = 0; i < WORD_SIZE; i++) {
            Byte b = memory.get(address + i);
            if (b == null) {
     ...
     * @throws SegmentFaultException
     */
    public void storeWord(final long address, final long value) throws SegmentFaultException {
        boundsCheck(address, WORD_SIZE);
        buffer.putWord(0, (int) value);
        for (int i = 0; i < WORD_SIZE; i++) {
            storeByte(address + i, buffer.getByte(i));
        }
    }
     ...
     * @throws SegmentFaultException
     */
    public long loadByte(final long address) throws SegmentFaultException {
     ...
     * @throws SegmentFaultException
     */
    public void storeByte(final long address, final long value) throws SegmentFaultException {
        boundsCheck(address, BYTE_SIZE);
        buffer.putByte(0, (byte) value);
        memory.put(address, buffer.getByte(0));
    }
